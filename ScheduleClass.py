...
class ScheduleClass:
    ...
    def getJsonFromKoleo(self, direction='A', offset='0'):
        ...
        if direction == 'A':
            d = {'query[start_station]': 'gdansk-rebiechowo',
                 'query[end_station]': 'gdansk-wrzeszcz',
                 'query[date]': date}
            dir = 'Kierunek Gdansk'
        else:
            d = {'query[start_station]': 'gdansk-rebiechowo',
                 'query[end_station]': 'gdynia-glowna',
                 'query[date]': date}
            dir = 'Kierunek Gdynia'

        try:
            data = requests.get(req, headers=h, data=d, verify=False,
                                timeout=10)
