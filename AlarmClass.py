...
class AlarmClass(object):
    ...
    def __get_ip(self, iface='eth0'):
        ...
        ifreq = struct.pack('16sH14s', iface, socket.AF_INET, '\x00'
                            * 14)
        try:
    ...
    def __get_netmask(self, iface='eth0'):
        netmask = \
            socket.inet_ntoa(fcntl.ioctl(socket.socket(socket.AF_INET,
    ...
    def toggleSwitchState(self, ip):
        ...
        output = requests.post(req, data=json.dumps(post_data),
                               headers=SwitchClass.__headers,
    ...
    def getTemperature(self):
        ...
        try:
            ...
            data['error'] = 0
            elements = []
            for item in xml.getElementsByTagName('sensors'
                    )[0].getElementsByTagName('sensor'):
                element = {}
                element['name'] = item.getElementsByTagName('sensorName'
                        )[0].firstChild.nodeValue
                element['value'] = \
                    item.getElementsByTagName('temperature'
                        )[0].firstChild.nodeValue
                element['thresholdExceeded'] = \
                    item.getElementsByTagName('thresholdExceeded'
                        )[0].firstChild.nodeValue
    ...
    def getAlerts(self):
        ...
        try:
            ...
            for item in xml.getElementsByTagName('sensors'
                ...
                if int(item.getElementsByTagName('alert'
                       )[0].firstChild.nodeValue) == 0:
                    element['alert'] = 'off'
                else:
                    element['alert'] = 'on'
                elements.append(element)
    ...
    def getPresence(self):
        ...
        try:
            ...
            for item in xml.getElementsByTagName('sensors'
                ...
                if int(item.getElementsByTagName('presenceState'
                       )[0].firstChild.nodeValue) == 0:
    ...
    def getStateSensors(self):
        ...
        try:
            ...
            for item in xml.getElementsByTagName('sensors'
                ...
                if int(item.getElementsByTagName('outputState'
                       )[0].firstChild.nodeValue) == 0:
                    element['output'] = 'off'
                else:
                    element['output'] = 'on'
    ...
    def getUpdateUrl(self, sensor, value):
        ...
        alarm = config.getAlarmSystem()
        req = 'http://' + alarm['ip'] + ':' + alarm['port'] \
            + '/command=UpdateSensorState&sensorName=' + sensor \
            + '&reading=' + str(value)
        return req
